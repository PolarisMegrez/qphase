"""Generate missing control-pair campaigns for the Collective Kerr dimer."""
...
PUMP = Dimension("pump_bright", 0.01, 0.995, logarithmic=False)
...
SCHEMES = (
    ...
    Scheme("nonlinear_dark", ("chi", "kappa_dark"), (DELTA, G, PUMP)),
)
...
def _rows(scheme: Scheme) -> list[dict[str, Any]]:
    ...
    strata = CHI_STRATA if scheme.stratify_chi else (None,)
...
def _job(scheme: Scheme, stage: str, rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    plugins: dict[str, Any] = {
        ...
        "model": {"collective_kerr_2mode": DEFAULTS},
    ...
    return {
        "name": f"collective_kerr_2mode_{scheme.name}_{stage.lower()}",
        "save": True,
        "system": {
            "scan_runtime": {
                "checkpoint": {
        ...
        "scan": {
            ...
            "axes": {
                name: {
                    "target": f"model.collective_kerr_2mode.{name}",
...
def main() -> None:
    ...
    for scheme in SCHEMES:
        ...
        manifest = (
            args.reports_dir
            / f"collective_kerr_2mode_{scheme.name}_manifest_2026-08-08.csv"
        )
        ...
        for stage in args.stages:
            path = (
                args.jobs_dir
                / f"collective_kerr_2mode_{scheme.name}_{stage.lower()}.yaml"
            )
