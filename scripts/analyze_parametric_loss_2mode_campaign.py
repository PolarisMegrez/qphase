"""Summarize candidates and near misses from parametric-loss campaigns."""
...
def _scheme(path: Path) -> str:
    stem = path.stem
    value = stem.removeprefix(f"{MODEL}_")
            ...
            rows.append(
                {
                    ...
                    **{name: params.get(name, math.nan) for name in fixed},
                    **{
                        name: value
