"""
QPhaseSDE: Torch State Containers
---------------------------------
...
class State(StateLike):
    ...
    --------
    >>> import torch
    >>> from QPhaseSDE.states.torch_state import State
    ...
    def copy(self) -> "State":
        ...
        return State(y=self.y.clone(), t=self.t, attrs=self.attrs.copy(), backend=self.backend)
    ...
    def to_backend(self, target_backend: Any, *, copy_if_needed: bool = True) -> Any:
        ...
        if name in ("numpy", "np", ""):
            ...
            return _NpState(y=arr.copy() if copy_if_needed else arr, t=self.t, attrs=self.attrs.copy(), backend=target_backend)
        # Otherwise, already on torch backend
        return State(y=self.y.clone() if copy_if_needed else self.y, t=self.t, attrs=self.attrs.copy(), backend=target_backend)
...
class TrajectorySet(TrajectorySetLike):
    ...
    --------
    >>> import torch
    >>> from QPhaseSDE.states.torch_state import TrajectorySet
