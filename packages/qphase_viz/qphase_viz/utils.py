"""qphase_sde: Visualizer utilities
-------------------------------
...
def spec_short_hash(spec: dict[str, Any], length: int = 8) -> str:
    ...
    The hash is generated using only the content of the spec (for example
    ``kind``, ``modes``, ``t_range``). Styling and cosmetic fields are
    ignored so that visual changes do not affect the identity.
...
def _time_to_index(
    t0: float, dt: float, n_steps: int, t_range: tuple[float, float] | None
):
...
def _ensure_outdir(run_dir: Path, out_dir: Path | None = None) -> Path:
