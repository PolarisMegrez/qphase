...
!!! warning "提案 —— qphase 2.0"
    本页描述的契约是 qphase/qphase_sde 2.0 升级的**提案性、实验性**设计，实现位于
    `qphase.data` 与 `qphase.core.task_profile`。该设计**尚未**获批冻结：在任何生产
    Result 序列化改动之前，本页列出的字段决议需经人工审核。
...
- `AxisSchema`——`name`、`role`、可选 `size`、`coordinate`（`regular` 或 `explicit`）、
  ...
  集合）、`realization`（不确定度统计所依赖的独立实现）、`coordinate`、`component` 与
  `index`。
...
- `UncertaintySchema`——`target`（所描述的变量）、`kind`（`sample_std`、`sem`、
  `confidence_interval`、`covariance`、`other`）、`independent_unit`（估计所统计的
  *realization* 轴——引用 parameter/coordinate 轴会被拒绝）、可选的资源包自定义
...
提案的最小 `SpectralQuantity` 集合为 `fourier_amplitude`、`power_spectral_density`、
...
- `RuntimeProductBacking`——产品的运行时支撑：每个 schema 变量恰好一个 handle，由
  `validate_backing` 校验（变量缺失/多余、dtype 与闭合轴 shape 不符都会被拒绝）。
- `ArtifactRef`——持久化、可跨进程引用，只携带身份：artifact id、产品 schema、
  `module:attr` loader、content hash 与 hash 算法。无 provenance、无数组、无额外字段。
...
- 可选的 profile resolver 只接收受限的 `TaskProfileResolutionContext`（规范化 job 配置
  与输入产品的 **schema**——绝不是 handle、payload、loader 或 scheduler），并返回
