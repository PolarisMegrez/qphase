# Test: CLI run with params sweep (cartesian)
import sys
...
def main():
    cfg_path = ROOT / 'tests' / 'tmp_cartesian.yaml'
    # Minimal sweep config (cartesian)
    cfg_text = f"""
profile:
  backend: numpy
  solver: euler
...
jobs:
  - name: sweep_cart
    ...
    noise:
      kind: independent
    # combinator omitted -> defaults to cartesian
"""
    ...
    assert len(new) == 4, f"Expected 4 runs for cartesian sweep, got {len(new)}"
    print('OK: CLI cartesian sweep produced 4 runs:', ', '.join(new))
