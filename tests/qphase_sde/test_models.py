...
def test_vdp_equations():
    ...
    expected = np.array(
        [
            [
                (-0.2j + 0.2 + 0.1 * (1 - abs(alpha) ** 2)) * alpha - 0.5j * beta,
    ...
    np.testing.assert_allclose(
        np.diagonal(diffusion, axis1=1, axis2=2) ** 2, [[1.1, 0.3]]
    )
    ...
    expected = np.asarray(
        [
            [
                ...
                (-1j * model.params["omega_b"] - model.params["gamma_b"] / 2.0) * beta
...
def test_kerr_3mode_gain_loss_signs_and_diffusion():
    ...
    diffusion = model.diffusion(y, 0.0, model.params)
    np.testing.assert_allclose(
        np.diagonal(diffusion, axis1=1, axis2=2) ** 2, [[0.2, 0.3, 0.4]]
    )
