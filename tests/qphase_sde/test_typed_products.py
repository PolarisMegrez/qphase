...
from qphase.data import AxisRole, DataKind, SpectralDataset, StatisticsDataset
...
def _run(model, analysers, *, scan=False, t1=0.2, n_traj=8):
...
def test_psd_scan_product_stacks_over_scan_axis():
    ...
    # Coordinate backing variables gained the scan axis, so no explicit
    # CoordinateSchema is declared on the stacked product.
    assert product.schema.coordinates == []
    assert product.attributes["graph_ready"] is True
