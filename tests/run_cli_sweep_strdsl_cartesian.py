# Test: CLI run with string-DSL sweeps (cartesian)
import sys
...
def main():
    cfg_path = ROOT / 'tests' / 'tmp_strdsl_cartesian.yaml'
    cfg_text = """
profile:
  backend: numpy
  solver: euler
...
"""
    ...
    assert len(new) == 4, f"Expected 4 runs (2x2) for string lin + array, got {len(new)}"
    print('OK: CLI string-DSL cartesian produced 4 runs:', ', '.join(new))
