# Test: CLI run with string-DSL sweeps (zipped)
import sys
...
def main():
    cfg_path = ROOT / 'tests' / 'tmp_strdsl_zipped.yaml'
    cfg_text = """
profile:
  backend: numpy
  solver: euler
...
"""
    ...
    assert len(new) == 3, f"Expected 3 runs for zipped string DSL, got {len(new)}"
    print('OK: CLI string-DSL zipped produced 3 runs:', ', '.join(new))
