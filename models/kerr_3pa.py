...
class Kerr3PAModel:
    ...
    def noise_basis(self) -> str:
        # We handle complex noise explicitly via 2 real channels to
        # support anomalous diffusion M
        return "real"
    ...
    def diffusion(self, y: Any, t: float, p: dict[str, Any]) -> Any:
        ...
        Uses soft-clipping of anomalous diffusion M to ensure positive
        semi-definiteness.
        """
        xp = get_xp(y)
        alpha = y[:, 0]
        ...
        # The P-representation Langevin equation is only valid when the
        # diffusion matrix is positive semi-definite, which requires D >= |M|.
        # In numerical simulations, fluctuations can push the state into
        # invalid regions. We regularize M to enforce PSD: if |M| > D,
        # scale M down.
        ...
        if xp.any(violation_mask):
            # Log debug warning (once per call to avoid flooding, or
            # checking first element). To be efficient on GPU, we sync
            # only if needed? For debugging, we can print/log, but
            # pulling from GPU is slow.
            ...
            try:
                ...
                if max_diff_val > 1e-6:
                    logger.debug(
                        f"[kerr_3pa] PSD violation: max(|M|-D)={max_diff_val:.4e}."
                        f" Scaling applied."
                    )
            except Exception:
                pass

        # Apply scaling ONLY where violation occurs (or min logic
        # handles it effectively). Using min(1.0, D/|M|) is cleaner
        # and differentiable-ish. To be STRICT about "no modification
        # if satisfied":
        # scale = 1.0 where |M| <= D
        # scale = D/|M| where |M| > D
        ...
        # term_l22_sq should be positive theoretically, but numerical
        # noise might affect it
