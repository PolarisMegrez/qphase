...
class Kerr3PAModel:
    ...
    def noise_basis(self) -> str:
        # We handle complex noise explicitly via 2 real channels to support
        # anomalous diffusion M
        return "real"
    ...
    def diffusion(self, y: Any, t: float, p: dict[str, Any]) -> Any:
        ...
        # In numerical simulations, fluctuations can push the state into invalid
        # regions.
        ...
        if xp.any(violation_mask):
            # Log debug warning (once per call to avoid flooding,
            # or checking first element)
            # To be efficient on GPU, we sync only if needed?
            # For debugging, we can just print/log. But pulling from GPU is
            # slow.
        ...
        # Apply scaling ONLY where violation occurs
        # (or min logic handles it effectively)
        ...
        # term_l22_sq should be positive theoretically,
        # but numerical noise might affect it
