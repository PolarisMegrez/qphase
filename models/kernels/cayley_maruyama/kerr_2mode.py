"""CuPy Cayley-Maruyama kernels for the two-mode Kerr model."""
...
static __device__ __forceinline__ $CT$ c_add_$S$($CT$ a, $CT$ b) {
    return c_make_$S$(a.x + b.x, a.y + b.y);
}

static __device__ __forceinline__ $CT$ c_sub_$S$($CT$ a, $CT$ b) {
    return c_make_$S$(a.x - b.x, a.y - b.y);
}

static __device__ __forceinline__ $CT$ c_mul_$S$($CT$ a, $CT$ b) {
    return c_make_$S$(a.x * b.x - a.y * b.y, a.x * b.y + a.y * b.x);
}

static __device__ __forceinline__ $CT$ c_div_$S$($CT$ a, $CT$ b) {
...
static __device__ __forceinline__ void advance_kerr_2mode_$S$(
    $CT$* alpha,
    $CT$* beta,
    const $T$* noise,
    $T$ omega_a,
    ...
    $CT$ drift_a = c_make_$S$(
        ($T$)0.5 * gamma_a * a.x + frequency_a * a.y + coupling * b.y,
        ($T$)0.5 * gamma_a * a.y - frequency_a * a.x - coupling * b.x
    );
    $CT$ drift_b = c_make_$S$(
        -($T$)0.5 * gamma_b * b.x + omega_b * b.y + coupling * a.y,
        -($T$)0.5 * gamma_b * b.y - omega_b * b.x - coupling * a.x
    );

    $T$ sigma_a = sqrt(gamma_a / ($T$)4.0);
    $T$ sigma_b = sqrt(gamma_b / ($T$)4.0);
    $CT$ eta_a = c_make_$S$(sigma_a * noise[0], sigma_a * noise[2]);
    $CT$ eta_b = c_make_$S$(sigma_b * noise[1], sigma_b * noise[3]);
    $CT$ rhs_a = c_add_$S$(
        c_add_$S$(a, c_make_$S$(half_dt * drift_a.x, half_dt * drift_a.y)), eta_a
    );
    $CT$ rhs_b = c_add_$S$(
        c_add_$S$(b, c_make_$S$(half_dt * drift_b.x, half_dt * drift_b.y)), eta_b
    );
    ...
    $CT$ m01 = c_make_$S$(($T$)0.0, half_dt * coupling);
    $CT$ m10 = m01;
    $CT$ m11 = c_make_$S$(
        ($T$)1.0 + half_dt * gamma_b / ($T$)2.0,
        half_dt * omega_b
    );
    $CT$ det = c_sub_$S$(c_mul_$S$(m00, m11), c_mul_$S$(m01, m10));
    *alpha = c_div_$S$(
        c_sub_$S$(c_mul_$S$(rhs_a, m11), c_mul_$S$(m01, rhs_b)), det
    );
    *beta = c_div_$S$(
        c_sub_$S$(c_mul_$S$(m00, rhs_b), c_mul_$S$(rhs_a, m10)), det
    );
}
"""
...
_STEP_SOURCE = (
    ...
    + r"""
extern "C" __global__
void __kerr_2mode_cayley_step_func__(
    ...
    $CT$ old_alpha = alpha;
    $CT$ old_beta = beta;
    advance_kerr_2mode_$S$(
        &alpha, &beta, &dW[i * 4],
        ($T$)omega_a[i], ($T$)omega_b[i], ($T$)chi[i],
        ($T$)gamma_a[i], ($T$)gamma_b[i], ($T$)g[i], dt
    );
    dy[i * 2 + 0] = c_sub_$S$(alpha, old_alpha);
    dy[i * 2 + 1] = c_sub_$S$(beta, old_beta);
}
"""
)
...
_CHUNK_SOURCE = (
    ...
    + r"""
extern "C" __global__
void __kerr_2mode_cayley_chunk_func__(
    const $CT$* __restrict__ y,
    const $T$* __restrict__ dW,
    const $T$* __restrict__ omega_a,
    const $T$* __restrict__ omega_b,
...
def fused_step(
    ...
    if tuple(y.shape) != (n, 2):
        raise ValueError(
            f"Kerr2 Cayley kernel expects state shape (n, 2), got {y.shape}"
        )
    if tuple(noise.shape) != (n, 4):
        raise ValueError(
            f"Kerr2 Cayley kernel expects noise shape (n, 4), got {noise.shape}"
        )
    rdtype = y.real.dtype
    source, ctype = _typed_source(_STEP_SOURCE, rdtype, "step")
    kernel = compile_cached_kernel("kerr_2mode_cayley_step", ctype, source)
    params_device = [
...
def fused_step_chunk(
    ...
    *,
    ...
    """Advance a fixed two-mode Kerr chunk and return final and saved states."""
    del backend
    import cupy as cp
    ...
    if any(mode not in (0, 1) for mode in record_modes):
        raise ValueError("Kerr2 chunk record_modes must contain only 0 or 1")
    if tuple(noise.shape) != (n_steps, n, 4):
        raise ValueError(
            "Kerr2 Cayley chunk noise must have shape "
    ...
    kernel = compile_cached_kernel("kerr_2mode_cayley_chunk", ctype, source)
    params_device = [
        broadcast_param(params[name], n, rdtype) for name in _PARAMETER_NAMES
    ]
    d_w = cp.asarray(noise, dtype=rdtype)
...
class Kerr2ModeCayleyCuPyKernel(ModelKernelPlugin):
    """CuPy fused-step provider for two-mode Kerr Cayley-Maruyama."""
