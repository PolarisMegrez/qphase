"""CuPy Cayley-Maruyama kernels for the three-mode Kerr model."""
...
_DEVICE_SOURCE = r"""
static __device__ __forceinline__ $CT$ c3_make_$S$($T$ x, $T$ y) {
    $CT$ z; z.x = x; z.y = y; return z;
}

static __device__ __forceinline__ $CT$ c3_add_$S$($CT$ a, $CT$ b) {
    return c3_make_$S$(a.x + b.x, a.y + b.y);
}

static __device__ __forceinline__ $CT$ c3_sub_$S$($CT$ a, $CT$ b) {
    return c3_make_$S$(a.x - b.x, a.y - b.y);
}

static __device__ __forceinline__ $CT$ c3_mul_$S$($CT$ a, $CT$ b) {
    return c3_make_$S$(a.x * b.x - a.y * b.y, a.x * b.y + a.y * b.x);
}

static __device__ __forceinline__ $CT$ c3_div_$S$($CT$ a, $CT$ b) {
...
static __device__ __forceinline__ void advance_kerr_3mode_$S$(
    $CT$* alpha,
    $CT$* beta,
    $CT$* gamma,
    const $T$* noise,
    ...
    $CT$ c = *gamma;
    $T$ half_dt = ($T$)0.5 * dt;
    $T$ frequency_a = omega_a + ($T$)2.0 * chi * (
        a.x * a.x + a.y * a.y - ($T$)1.0
    );

    $CT$ drift_a = c3_make_$S$(
        -($T$)0.5 * gamma_a * a.x + frequency_a * a.y
            + coupling_ab * b.y + coupling_ac * c.y,
        -($T$)0.5 * gamma_a * a.y - frequency_a * a.x
    ...
    $T$ sigma_c = sqrt(gamma_c / ($T$)4.0);
    $CT$ eta_a = c3_make_$S$(sigma_a * noise[0], sigma_a * noise[3]);
    $CT$ eta_b = c3_make_$S$(sigma_b * noise[1], sigma_b * noise[4]);
    $CT$ eta_c = c3_make_$S$(sigma_c * noise[2], sigma_c * noise[5]);
    $CT$ rhs_a = c3_add_$S$(
        c3_add_$S$(a, c3_make_$S$(half_dt * drift_a.x, half_dt * drift_a.y)), eta_a
    );
    $CT$ rhs_b = c3_add_$S$(
        c3_add_$S$(b, c3_make_$S$(half_dt * drift_b.x, half_dt * drift_b.y)), eta_b
    );
    $CT$ rhs_c = c3_add_$S$(
        c3_add_$S$(c, c3_make_$S$(half_dt * drift_c.x, half_dt * drift_c.y)), eta_c
    );
    ...
    $CT$ m01 = c3_make_$S$(($T$)0.0, half_dt * coupling_ab);
    $CT$ m02 = c3_make_$S$(($T$)0.0, half_dt * coupling_ac);
    $CT$ m10 = m01;
    $CT$ m11 = c3_make_$S$(
    ...
    *gamma = c3_div_$S$(numerator_c, det);
}
"""
...
void __kerr_3mode_cayley_step_func__(
    ...
    dy[i * 3 + 2] = c3_sub_$S$(gamma, old_gamma);
}
"""
)
...
_CHUNK_SOURCE = (
    ...
    + r"""
extern "C" __global__
void __kerr_3mode_cayley_chunk_func__(
    const $CT$* __restrict__ y,
    const $T$* __restrict__ dW,
    const $T$* __restrict__ omega_a,
    const $T$* __restrict__ omega_b,
    ...
    for (int step = 0; step < n_steps; ++step) {
        ...
        if (save_cursor < n_saves && step + 1 == save_offsets[save_cursor]) {
            ...
            for (int cursor = 0; cursor < n_record_modes; ++cursor) {
                ...
                saved[save_base + cursor] = (
                    mode == 0 ? alpha : (mode == 1 ? beta : gamma)
                );
            }
            ++save_cursor;
        }
    }
    final_state[i * 3 + 0] = alpha;
    final_state[i * 3 + 1] = beta;
    final_state[i * 3 + 2] = gamma;
}
"""
)
...
def _get_buffer(n: int, dtype: Any) -> Any:
    ...
    buffer = cp.empty((n, 3), dtype=dtype)
...
def fused_step(
    ...
    n = int(y.shape[0])
    if tuple(y.shape) != (n, 3):
        raise ValueError(
            f"Kerr3 Cayley kernel expects state shape (n, 3), got {y.shape}"
        )
    if tuple(noise.shape) != (n, 6):
        raise ValueError(
            f"Kerr3 Cayley kernel expects noise shape (n, 6), got {noise.shape}"
        )
    rdtype = y.real.dtype
...
def fused_step_chunk(
    ...
    *,
    ...
    """Advance a fixed three-mode Kerr chunk and return final and saved states."""
    del backend
    import cupy as cp
    ...
    if any(mode not in (0, 1, 2) for mode in record_modes):
        raise ValueError("Kerr3 chunk record_modes must contain only 0, 1, or 2")
    if tuple(noise.shape) != (n_steps, n, 6):
        raise ValueError(
            "Kerr3 Cayley chunk noise must have shape "
            f"({n_steps}, {n}, 6); got {noise.shape}"
        )
...
class Kerr3ModeCayleyCuPyKernel(ModelKernelPlugin):
    """CuPy fused-step provider for three-mode Kerr Cayley-Maruyama."""
